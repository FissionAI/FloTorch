...
class Indexer(BasePipeline):
    def execute(self) -> None:
        ...
        try:
            ...
            # Step 3: Embedding
            # TODO: Instead of bedrock_role_arn, use the role_arn from the config and based on the service_type
            if self.experimentalConfig.embedding_service == 'bedrock':
                embedding_results = EmbedProcessor(service_type=self.experimentalConfig.embedding_service,
                                                   model_id=self.experimentalConfig.embedding_model,
                                                   aws_region=self.config.aws_region,
                                                   role_arn=self.config.bedrock_role_arn,
                                                   vector_dimension=self.experimentalConfig.vector_dimension).embed(
                    chunks)
            else:
                embedding_results = EmbedProcessor(service_type=self.experimentalConfig.embedding_service,
                                                   model_id=self.experimentalConfig.embedding_model,
                                                   ...
                                                   vector_dimension=self.experimentalConfig.vector_dimension).embed(
                    chunks)
            documents = self.prepare_documents(embedding_results.embed_list, chunks, embedding_results.input_tokens)
    ...
    def _insert_to_opensearch(self, documents):
        vector_database = OpenSearchVectorDatabase(host=self.config.opensearch_host,
